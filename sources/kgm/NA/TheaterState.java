...
public class TheaterState extends State {
    int fadeAlpha;
    boolean isFading;
    States nextState;

    public TheaterState(AssetManager am, Assets assets) {
    ...
    public void update(MotionEvent touch, KeyEvent key) {
        ...
        if (touch != null && !this.isFading) {
            if (touch.getY() > (Screen.relScreenHeight * 0.7f) + Screen.relYZero && touch.getY() < (Screen.relScreenHeight * 0.8833f) + Screen.relYZero && touch.getX() > (Screen.relScreenWidth * 0.7f) + Screen.relXZero && touch.getX() < Screen.relScreenWidth + Screen.relXZero) {
    ...
    public void draw(Canvas canvas) {
        super.draw(canvas);
        canvas.drawBitmap(this.assets.theater.bitmap, Screen.relXZero, Screen.relYZero, (Paint) null);
        canvas.drawARGB(this.fadeAlpha, 0, 0, 0);
    }

    @Override // kgm.NA.State
    public void load(AssetManager am, Assets assets) {
        assets.theater.load(am, "theater.png", 400, 300);
